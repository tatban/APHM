...
import warnings
import torch
from  renderer.rend_utils import get_sphere_intersections
...
class UniformSampler(RaySampler):
    ...
    def get_z_vals(self, ray_dirs, cam_loc, model, shape_code):
        if not self.take_sphere_intersection:
            ...
            near = self.near * torch.ones(ray_dirs.shape[0], ray_dirs.shape[1], 1).cuda()
...
class ErrorBoundSampler(RaySampler):
    def __init__(self, scene_bounding_sphere, near=0.2, far=5.0, N_samples=128, N_samples_eval=128, N_samples_extra=32,
                 eps=0.1, beta_iters=10, max_total_iters=5,
                 inverse_sphere_bg=False, N_samples_inverse_sphere=0, add_tiny=1.0e-6):
        super().__init__(near, far)

        self.N_samples = N_samples
        self.N_samples_eval = N_samples_eval
        self.uniform_sampler = UniformSampler(scene_bounding_sphere, near, N_samples_eval, take_sphere_intersection=True, far=far)  # replica scannet and T&T courtroom
    ...
    def get_z_vals(self, ray_dirs, cam_loc, model, shape_code):
        ...
        z_vals, near, far = self.uniform_sampler.get_z_vals(ray_dirs, cam_loc, model, shape_code)  # model and shape_code are not actually used for query here, but passed to satisfy the function signature. todo: refactor
        samples, samples_idx = z_vals, None  # dim: bs*n_pixel x n_samples

        # Get maximum beta from the upper bound (Lemma 2)
        dists = z_vals[:, :, 1:] - z_vals[:, :, :-1]  # dim: bs*n_pixel x n_samples-1
        bound = (1.0 / (4.0 * torch.log(torch.tensor(self.eps + 1.0)))) * (dists ** 2.).sum(-1)  # dim: bs x n_pixel
        beta = torch.sqrt(bound)  # dim: bs x n_pixel

        total_iters, not_converge = 0, True

        # Algorithm 1
        while not_converge and total_iters < self.max_total_iters:
            points = cam_loc.unsqueeze(2) + samples.unsqueeze(3) * ray_dirs.unsqueeze(2)  # dim: bs*n_pixel*n_sample x 3
            points_flat = points.reshape(ray_dirs.shape[0], -1, 3)  # dim: bs x (n_pixels*depth_samples) x 3

            # Calculating the SDF only for the new sampled points
            with torch.no_grad():
                samples_sdf = model.implicit_network.get_sdf_vals(points_flat, shape_code)
            if samples_idx is not None:
                sdf_merge = torch.cat([sdf.reshape(z_vals.shape[0], -1, z_vals.shape[2] - samples.shape[2]),
                                       samples_sdf.reshape(samples.shape[0], -1, samples.shape[2])], -1)
                sdf = torch.gather(sdf_merge, 2, samples_idx).reshape(z_vals.shape[0], -1, 1)
            else:
                sdf = samples_sdf  # dim: bs x (n_pixels*depth_samples) x 3
            ...
            beta_min, beta_max = beta0.unsqueeze(0).unsqueeze(1).repeat(z_vals.shape[0], z_vals.shape[1]), beta
            for j in range(self.beta_iters):
                beta_mid = (beta_min + beta_max) / 2.
                curr_error = self.get_error_bound(beta_mid.unsqueeze(-1), model, sdf, z_vals,
                                                  dists, d_star)
            ...
            dists = torch.cat(
                [dists, torch.tensor([1e10]).cuda().unsqueeze(0).unsqueeze(1).repeat(dists.shape[0], dists.shape[1], 1)],
                -1)
            free_energy = dists * density
            shifted_free_energy = torch.cat(
                [torch.zeros(dists.shape[0], dists.shape[1], 1).cuda(), free_energy[:, :, :-1]], dim=-1)
            ...
            if not_converge and total_iters < self.max_total_iters:
                ...
                bins = z_vals
                error_per_section = torch.exp(-d_star / beta.unsqueeze(-1)) * (
                            dists[:, :, :-1] ** 2.) / (4 * beta.unsqueeze(-1) ** 2)
                error_integral = torch.cumsum(error_per_section, dim=-1)
                bound_opacity = (torch.clamp(torch.exp(error_integral),
                                             max=1.e6) - 1.0) * transmittance[:, :, :-1]
            ...
            if (not_converge and total_iters < self.max_total_iters) or (not model.training):
                u = torch.linspace(0., 1., steps=N).cuda().unsqueeze(0).unsqueeze(1).repeat(cdf.shape[0], cdf.shape[1], 1)
            else:
        ...
        if not_converge and total_iters>=self.max_total_iters:
            warnings.warn("Sampling iteration didn't converge.")
        z_samples = samples
        near, far = self.near * torch.ones(ray_dirs.shape[0], ray_dirs.shape[1], 1).cuda(), self.far * torch.ones(
            ray_dirs.shape[0], ray_dirs.shape[1], 1).cuda()
        ...
        if self.N_samples_extra > 0:
            if model.training:
                ...
                sampling_idx = torch.linspace(0, z_vals.shape[2] - 1,
                                              self.N_samples_extra).long()
        ...
        idx = torch.randint(z_vals.shape[-1], (z_vals.shape[0], z_vals.shape[1],)).cuda()
    ...
    def get_error_bound(self, beta, model, sdf, z_vals, dists, d_star):
        ...
        shifted_free_energy = torch.cat([torch.zeros(dists.shape[0], dists.shape[1], 1).cuda(), dists * density[:, :, :-1]], dim=-1)
