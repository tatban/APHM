# adapted from pi-GAN: https://raw.githubusercontent.com/marcoamonteiro/pi-GAN/master/discriminators/discriminators.py
"""Discriminator used in APHM"""
...
class NormalMapDiscriminator(nn.Module):
    ...
    def forward(self, input, alpha=1, **kwargs):  # alpha controls the progressive growing, we set alpha=1 to disable progressive growing (modified)
        start = self.img_size_to_layer[input.shape[-1]]

        x = self.fromRGB[start](input)
        for i, layer in enumerate(self.layers[start:]):
            if i == 1 and alpha != 1:  # modified
